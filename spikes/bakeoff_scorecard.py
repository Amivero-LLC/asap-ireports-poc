...
SCORECARD = Scorecard(
    ...
    recommendation_rationale=(
        ...
        "Two qualifications the program should carry forward. The first has since been "
        "settled: cold start and packaging under SAM local were the one measurement "
        "most likely to reopen this, and ADR-023 took it — LangGraph imports in roughly 1.6-2.3s "
        "against ~0.5s for the framework-free control, about 3x, at 19 MB zipped against a 50 MB "
        "limit. On a workload where one specialist model call runs tens of seconds, that does not "
        "reopen the choice. The figures are load-sensitive and are a comparison rather than a "
        "production number; see spikes/lambda_fit/. And the "
